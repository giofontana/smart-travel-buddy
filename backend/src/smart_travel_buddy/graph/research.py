...
async def call_weather(state: TravelState, config: RunnableConfig) -> dict[str, Any]:
    ...
    if weather_tool:
        ...
        try:
            ...
            result = json.dumps({"error": str(e), "city": city})
...
async def call_currency(state: TravelState, config: RunnableConfig) -> dict[str, Any]:
    ...
    if to_currency == "USD":
        ...
        try:
            ...
            result = json.dumps({"error": str(e), "from": "USD", "to": to_currency})
...
async def call_wikipedia(state: TravelState, config: RunnableConfig) -> dict[str, Any]:
    ...
    if wikipedia_tool:
        ...
        try:
            ...
            result = json.dumps({"error": str(e), "topic": city})
    ...
    if db_session:
        try:
            ...
            with mlflow_span("rag-retrieval", span_type="RETRIEVER", attributes={
                ...
                chunks = await retrieve_chunks(db_session, query, top_k=5)
